// cloudfunctions/tag/index.js — 标签提交与自动归类
...
const {
  ...
  getUserByOpenId, requireRole, generateId
...
exports.main = async (event, context) => {
  ...
  try {
    ...
    switch (action) {
      // ---- 标签配置（管理员） ----
      case 'configList':
        return await handleConfigList(data, user)
      case 'configCreate':
        return await handleConfigCreate(data, user)
      case 'configUpdate':
        return await handleConfigUpdate(data, user)
      case 'configToggle':
        return await handleConfigToggle(data, user)

      // ---- 标签提交（班主任） ----
      case 'submit':
        return await handleSubmit(data, user)
      case 'remove':
        return await handleRemove(data, user)

      // ---- 学生标签历史 ----
      case 'studentTagHistory':
        return await handleStudentTagHistory(data, user)

      // ---- 统计（按分类体系） ----
      case 'categoryStats':
        return await handleCategoryStats(data, user)
...
async function handleConfigList(data, user) {
  ...
  const list = await db.collection('config_tags')
    .where(where)
    .orderBy('group', 'asc')
...
async function handleConfigCreate(data, user) {
  ...
  if (!group) return fail(400, '展示分组不能为空')
...
async function handleConfigToggle(data, user) {
  ...
  await db.collection('config_tags').doc(configId).update({
    data: { is_enabled: isEnabled, update_time: Date.now() }
  })
...
async function handleSubmit(data, user) {
  ...
  // 获取标签映射配置
  const tagConfigs = await db.collection('config_tags')
    .where({ name: _.in(tagNames), is_enabled: true })
    .get()

  if (tagConfigs.data.length === 0) {
    return fail(400, '所选标签不存在或已禁用')
  }
  ...
  if (invalidNames.length > 0) {
    return fail(400, `以下标签不存在或已禁用: ${invalidNames.join(', ')}`)
  }
  ...
  if (newConfigs.length === 0) {
    return fail(422, '所选标签已全部存在，无需重复添加')
  }
  ...
  // 如果添加了在籍不在校相关标签，更新状态
  const outCategories = newConfigs
    .filter(c => c.category_out)
    .map(c => c.category_out)

  if (outCategories.length > 0 && !outOfSchoolStatus) {
  ...
  return success({
    ...
    classification: {
      six: sixCategories,
      four: fourCategories,
      out_of_school: outCategoriesResult
    },
    outOfSchoolStatus
...
async function handleStudentTagHistory(data, user) {
  ...
  const list = await db.collection('student_tags')
    .where(where)
...
async function handleCategoryStats(data, user) {
  ...
  const students = await db.collection('students')
    .where(where)
    .field({ tags: true, out_of_school_status: true, class_id: true, grade_id: true })
    .get()

  const stats = {
    six: {},      // { '家庭类': 12, '心理类': 5, ... }
    four: {},     // { '特殊家庭': 10, '特异心理': 3, ... }
    outOfSchool: {} // { '长时间请假': 2, '疑似辍学': 1, ... }
  }
